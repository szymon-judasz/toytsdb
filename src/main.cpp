...
void send(io_uring* ring, int client_fd, const char* buffer, size_t buffer_size) {
    io_uring_sqe *sqe = io_uring_get_sqe(ring);
...
int main() {
    ...
    while (true) {
        ...
        // case send, get send size from cqe res, advance pointers and try again sending, if all sent -> spawn receive sqe
                break;
        default:
            break;
        }
